...
//import actions
import { googleSignIn, signOut } from '~/store/actions/account'
import { useSelector } from 'react-redux'

const useResumeSession = () => {
    const dispatch = useDispatch()
    const currentUser = useSelector(({ accountReducer: { currentUser }}) => currentUser)
    console.log('ran')
    ...
    useEffect(() => {
        const unsubscribeFromAuth = auth.onAuthStateChanged((userAuth) => {
            ...
            if (!userAuth) return
            return
            const userAuthFromEmailAndPasswordCreation = userAuth.hasOwnProperty('additionalUserInfo') && userAuth.additionalUserInfo.isNewUser && userAuth.hasOwnProperty('additionalUserInfo') && userAuth.operationType === "signIn"
                ...
                userDocRef.onSnapshot((snapshot) => {
                    if (snapshot.exists) {
                        ...
                        if (!userAuthFromEmailAndPasswordCreation) {
                            const { email, displayName, photoURL, phoneNumber, emailVerified, uid } = snapshot.data()

                            dispatch(googleSignIn({ uid, email, displayName, photoURL, phoneNumber, emailVerified }))
                        }
                    } else {
                        dispatch(signOut())
                    }
